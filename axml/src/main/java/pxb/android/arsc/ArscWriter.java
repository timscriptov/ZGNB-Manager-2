...
public class ArscWriter implements ResConst {
    ...
    private void travelBagValue(@NotNull BagValue bag) {
        for (Map.Entry<Integer, Value> e : bag.map) {
            travelValue(e.getValue());
        }
    }

    private void travelValue(@NotNull Value v) {
        if (v.raw != null) {
            addString(v.raw);
        }
    }

    private void write(@NotNull ByteBuffer out, int size) throws IOException {
    ...
    private void writeValue(@NotNull Value value, @NotNull ByteBuffer out) {
