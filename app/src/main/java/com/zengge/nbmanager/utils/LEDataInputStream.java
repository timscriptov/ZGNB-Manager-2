...
public final class LEDataInputStream implements DataInput {
    ...
    public void skipCheckInt(int expected) throws IOException {
        ...
        if (got != expected)
            throw new IOException(String.format(
                    ...
                    new Object[]{Integer.valueOf(expected),
                            Integer.valueOf(got)
                    }));
    }
    ...
    public void skipCheckShort(short expected) throws IOException {
        ...
        if (got != expected)
            throw new IOException(
                    String.format(
                            "Expected: 0x%08x, got: 0x%08x",
                            new Object[]{Short.valueOf(expected),
                                    Short.valueOf(got)
                            }));
    }
    ...
    public void skipCheckByte(byte expected) throws IOException {
        ...
        if (got != expected)
            throw new IOException(String.format(
                    "Expected: 0x%08x, got: 0x%08x",
                    new Object[]{Byte.valueOf(expected), Byte.valueOf(got)}));
    }

    public @NotNull String readNulEndedString(int length, boolean fixed)
            throws IOException {
    ...
    public final long readLong() throws IOException {
        ...
        return work[7] << 56 | (work[6] & 0xFF) << 48 | (work[5] & 0xFF) << 40
                | (work[4] & 0xFF) << 32 | (work[3] & 0xFF) << 24
                | (work[2] & 0xFF) << 16 | (work[1] & 0xFF) << 8 | work[0]
                & 0xFF;
    }
