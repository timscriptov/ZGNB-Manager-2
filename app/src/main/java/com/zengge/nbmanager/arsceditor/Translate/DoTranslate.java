/**
 * Copyright 2015 com.zenggeHai <2801045898@qq.com>
 * <p>
...
package com.zengge.nbmanager.arsceditor.Translate;
...
public class DoTranslate implements OnCheckedChangeListener {
    ...
    // 跳过已翻译的内容的CheckBox控件
    private AppCompatCheckBox skip_already_translate;
    // 源语言，翻译为，翻译商选项的Spinner控件
    private AppCompatSpinner src_type, translate_to, translator;
    // 能够翻译的语言的集合
    ...
    // 构造函数
    public DoTranslate(List<String> source_list, List<String> target_list, boolean translate_all, @NotNull Context context) {
        // 获取需要翻译的内容的集合
    ...
    public void init(int _position) {
        ...
        new AlertDialog.Builder(mContext).setView(DialogView)// 设置自定义对话框的样式
                ...
                .setNegativeButton(R.string.translate, (p1, p2) -> {
                    // 开启一个翻译线程
                    new translate_task().execute(source_list, target_list);
                }).create()// 创建
                .show(); // 显示对话框
        // 找到显示源语言的Spinner控件
        src_type = (AppCompatSpinner) DialogView.findViewById(R.id.src_type);
        // 找到显示目标语言的Spinner控件
        translate_to = (AppCompatSpinner) DialogView.findViewById(R.id.translate_to);
        // 找到翻译商的选项的Spinner控件
        translator = (AppCompatSpinner) DialogView.findViewById(R.id.translator);
        // 找到显示跳过选项的CheckBox控件
        skip_already_translate = (AppCompatCheckBox) DialogView.findViewById(R.id.skip_already_translate);
        // 源语言默认自动识别
    ...
    class translate_task extends AsyncTask<List<String>, Void, String> {
        ...
        @SafeVarargs
        @Override
        protected final String doInBackground(List<String>... parms) {
            // 翻译的结果
            String translatedTexte;
            try {
                ...
                for (String item : parms[1]) { // 遍历获取listitem_change的内容
                    if (!item.equals("")) { // 如果内容不为空
                        ((ArscActivity) mContext).isChanged = true; // 字符已经改变的标志
                        break; // 跳出循环
                    }
                }
        ...
        protected void onPostExecute(String result) {
            ...
            if (result.equals(mContext.getString(R.string.translate_success)))
                Toast.makeText(mContext, result, Toast.LENGTH_SHORT).show();
            else
                ArscActivity.showMessage(mContext, result).show();// 显示错误信息
        }
    }
}
