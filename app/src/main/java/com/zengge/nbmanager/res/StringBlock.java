...
public class StringBlock {
    ...
    public static @NotNull StringBlock read(@NotNull LEDataInputStream reader) throws IOException {
    ...
    @Contract(pure = true)
    private static final int getShort(byte @NotNull [] array, int offset) {
        return (array[offset + 1] & 0xFF) << 8 | array[offset] & 0xFF;
    }

    @Contract(value = "_, _ -> new", pure = true)
    private static final int @NotNull [] getVarint(byte @NotNull [] array, int offset) {
    ...
    public void write(@NotNull List<String> list, LEDataOutputStream out)
            throws IOException {
    ...
    public String getString(int index) {
        if ((index < 0) || (this.m_stringOffsets == null)
    ...
    private void outputStyleTag(String tag, @NotNull StringBuilder builder, boolean close) {
        builder.append('<');
        if (close)
            builder.append('/');
        int pos = tag.indexOf(';');
    ...
    private int @Nullable [] getStyle(int index) {
        ...
        int i = offset;
        for (int j = 0; (i < this.m_styles.length) && (this.m_styles[i] != -1); )
            style[(j++)] = this.m_styles[(i++)];
        return style;
    }

    private @Nullable String decodeString(int offset, int length) {
        try {
            return (this.m_isUTF8 ? UTF8_DECODER : UTF16LE_DECODER).decode(
                    ByteBuffer.wrap(this.m_strings, offset, length)).toString();
