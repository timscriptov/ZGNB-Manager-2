package com.zengge.nbmanager.dalvik;

import com.zengge.nbmanager.code.utils.ByteRenderer;
import com.zengge.nbmanager.code.utils.IndentingWriter;
import com.zengge.nbmanager.code.utils.LiteralTools;
import com.zengge.nbmanager.code.utils.LongRenderer;
...
public class Parser {
    ...
    public static final Pattern pInt = Pattern.compile("\\s" + hex_literal
            + "|\\s([+,-])?\\d+");
    public static final Pattern pLong = Pattern.compile("\\s" + hex_literal
            + "([l,L])?|\\s([+,-])?\\d+([l,L])?");
    ...
    private static void writeInvokeRegister(@NotNull IndentingWriter writer,
    ...
    private static void writeLabel(@NotNull IndentingWriter writer, int off)
            throws IOException {
        writer.write(':');
        writer.write(LABEL);
    ...
    private static void writeKeyAndTarget(@NotNull IndentingWriter writer, int key,
    ...
    private static void writeToReferencedItem(IndentingWriter writer,
                                              @NotNull Instruction instruction) throws IOException {
    ...
    private static byte @NotNull [] parseFiveRegister(@NotNull String s, int[] len) {
    ...
    private static int @NotNull [] parseRangeRegister(@NotNull String s) {
    ...
    private static FieldIdItem parseField(DexFile dexFile, String s) {
        ...
        return FieldIdItem.internFieldIdItem(dexFile,
                TypeIdItem.internTypeIdItem(dexFile, classType),
                TypeIdItem.internTypeIdItem(dexFile, type),
                StringIdItem.internStringIdItem(dexFile, name));
    }
    ...
    private static MethodIdItem parseMethod(DexFile dexFile, String s) {
        ...
        ProtoIdItem proto = ProtoIdItem.internProtoIdItem(dexFile,
                TypeIdItem.internTypeIdItem(dexFile, returnType), params);
        return MethodIdItem.internMethodIdItem(dexFile,
                TypeIdItem.internTypeIdItem(dexFile, classType), proto,
                StringIdItem.internStringIdItem(dexFile, name));
    }
    ...
    private static TypeIdItem parseType(DexFile dexFile, String s) {
        ...
        return TypeIdItem.internTypeIdItem(dexFile, type);
    }

    private static StringIdItem parseString(DexFile dexFile, @NotNull String s) {
        int i = s.indexOf("\"");
        int j = s.lastIndexOf("\"");
        return StringIdItem.internStringIdItem(dexFile,
    ...
    private static int parseTarget(String s,
                                   HashMap<String, Integer> targetOffsets, int codeOffset) {
        try {
            String[] str = s.split(":");
    ...
    public static @Nullable TypeListItem buildTypeList(DexFile dexFile, @NotNull String str) {
        ...
        while (typeStartIndex < str.length()) {
            switch (str.charAt(typeStartIndex)) {
                ...
                case 'F':
                case 'D':
                    typeList.add(TypeIdItem.internTypeIdItem(dexFile,
    ...
    private static Pair<List<TryItem>, List<EncodedCatchHandler>> parseCatchs(
            DexFile dexFile, String @NotNull [] strings, int index,
            HashMap<String, Integer> labelsMapOffsets) {
        ...
        for (; index < strings.length; index++) {
            ...
            if (string.startsWith(".catch")) {
                ...
                if (sp.length < 2)
                    throw new IllegalArgumentException("no exception type : "
                            + strings[index]);
                ...
                if (exceptionType.equals(ALL))
                    ...
                    tryList.addHandler(
                            TypeIdItem.internTypeIdItem(dexFile, exceptionType),
                            start, end, handler);
                }
            }
        }
    ...
    public static boolean searchStringInMethod(
            ClassDataItem.@NotNull EncodedMethod method, String src) {
                ...
                switch (instruction.getFormat()) {
                    ...
                    case Format31c:
                        switch (instruction.opcode.referenceType) {
                            case string:
                                InstructionWithReference ref = (InstructionWithReference) instruction;
                                String string = ((StringIdItem) ref.getReferencedItem())
                                        .getStringValue();
    ...
    public static boolean searchMethodInMethod(
            ClassDataItem.@NotNull EncodedMethod method, String classType, String name,
                ...
                switch (instruction.getFormat()) {
                    ...
                    case Format3rc:
                        switch (instruction.opcode.referenceType) {
                            case method:
                                InstructionWithReference ref = (InstructionWithReference) instruction;
                                MethodIdItem item = ((MethodIdItem) ref
                                        .getReferencedItem());
                                if (ignoreNameAndDescriptor
                                        && item.getContainingClass()
                                        .getTypeDescriptor().equals(classType))
                                    return true;
                                if (ignoreDescriptor
                                        && item.getContainingClass()
                                        .getTypeDescriptor().equals(classType)
                                        && item.getMethodName().getStringValue()
                                        .equals(name))
                                    return true;
                                if (item.getContainingClass().getTypeDescriptor()
                                        .equals(classType)
                                        && item.getMethodName().getStringValue()
                                        .equals(name)
                                        && item.getPrototype().getPrototypeString()
                                        .equals(descriptor))
                                    return true;
                        }
                }
    ...
    public static boolean searchFieldInMethod(
            ClassDataItem.@NotNull EncodedMethod method, String classType, String name,
            String descriptor, boolean ignoreNameAndDescriptor,
            boolean ignoreDescriptor) {
        if (method.codeItem != null) {
                ...
                switch (instruction.getFormat()) {
                    ...
                    case Format22c:
                        switch (instruction.opcode.referenceType) {
                            case field:
                                InstructionWithReference ref = (InstructionWithReference) instruction;
                                FieldIdItem item = (FieldIdItem) ref
                                        .getReferencedItem();
                                if (ignoreNameAndDescriptor
                                        && item.getContainingClass()
                                        .getTypeDescriptor().equals(classType))
                                ...
                                if (item.getContainingClass().getTypeDescriptor()
                                        ...
                                        && item.getFieldName().getStringValue()
                                        .equals(name)
                                        && item.getFieldType().getTypeDescriptor()
                                        .equals(descriptor))
                                    return true;
                        }
                }
    ...
    public void dump(IndentingWriter writer) throws IOException {
        if (code == null)
            return;
        ...
        for (int i = 0, len = instructions.length; i < len; i++) {
            ...
            switch (instruction.getFormat()) {
                ...
                case Format10t:
                    ...
                    writer.write(' ');
                    writeLabel(writer,
                            currentCodeAddress + ins10x.getTargetAddressOffset());
                    writer.write('\n');
                    break;
                case Format11x:
                ...
                case Format21t:
                    ...
                    writer.write(' ');
                    writeLabel(writer,
                            currentCodeAddress + ins21t.getTargetAddressOffset());
                    writer.write('\n');
                    break;
                case Format21c:
                case Format31c:
                    ...
                    writer.write(' ');
                    writer.write('v');
                    writer.printIntAsDec(((SingleRegisterInstruction) instruction)
                            .getRegisterA());
                    writer.write(' ');
                ...
                case Format51l:
                    ...
                    writer.printIntAsDec(((SingleRegisterInstruction) instruction)
                            .getRegisterA());
                    writer.write(' ');
                    LongRenderer.writeSignedIntOrLongTo(writer,
                ...
                case Format22t:
                    ...
                    writer.write(' ');
                    writeLabel(writer,
                            currentCodeAddress + ins22t.getTargetAddressOffset());
                    writer.write('\n');
                    break;
                case Format22b:
                ...
                case Format31t:
                    ...
                    writer.write(' ');
                    writeLabel(writer,
                            currentCodeAddress + ins31t.getTargetAddressOffset());
                    writer.write('\n');
                    break;
                case Format32x:
                ...
                case SparseSwitchData: {
                    ...
                    writer.write('\n');
                    int switch_off = switchOpcodeOffsets.get(currentCodeAddress);
                    Iterator<SparseSwitchDataPseudoInstruction.SparseSwitchTarget> iterator = sswitch
                            .iterateKeysAndTargets();
                    writer.indent(4);
                    while (iterator.hasNext()) {
                        SparseSwitchDataPseudoInstruction.SparseSwitchTarget keyAndTarget = iterator
                                .next();
                        writeKeyAndTarget(writer, keyAndTarget.key, switch_off
                                + keyAndTarget.targetAddressOffset);
                        writer.write('\n');
                    }
                    writer.deindent(4);
                ...
                case ArrayData: {
                    ...
                    writer.write('\n');
                    writer.indent(4);
                    Iterator<ArrayDataPseudoInstruction.ArrayElement> elements = arrayData
                            .getElements();
                    while (elements.hasNext()) {
                        ArrayDataPseudoInstruction.ArrayElement element = elements
                                .next();
                        for (int width = element.bufferIndex; width < element.elementWidth
                                + element.bufferIndex; width++) {
                            ByteRenderer.writeUnsignedTo(writer,
                                    element.buffer[width]);
                            writer.write(' ');
                        }
                        writer.write('\n');
                    }
    ...
    public void parse(DexFile dexFile, String string) throws Exception {
        ...
        final HashMap<String, Integer> switchOpcodeOffsets = new HashMap<String, Integer>(
                1);
        int currentCodeAddress = 0;
        ...
        for (int i = 0, len = strings.length; i < len; i++) {
            ...
            if (opcode == null) {
                ...
                if (opcodeName.equals(".pswitch_data")) {
                    ...
                    int switchOpcodeOffset = switchOpcodeOffsets.get(switchOff
                            .substring(0, switchOff.length() - 1));
                    ...
                    while (true) {
                        ...
                        if (sp.length != 2) {
                            throw new IllegalArgumentException(
                                    "packed switch data error: "
                                            + nextKeyAndTarget);
                        }
                        try {
                            keysAndTargets.put(Integer.parseInt(sp[0].trim()),
                                    labelsMapOffsets.get(sp[1].trim())
                                            - switchOpcodeOffset);
                        } catch (Exception e) {
                            throw new IllegalArgumentException(
                                    "the key is not int or label is not exists: "
                                            + nextKeyAndTarget);
                        }
                    }
                    int firstKey = keysAndTargets.keyAt(0);
                    int[] targetsArray = keysAndTargets.values();
                    PackedSwitchDataPseudoInstruction packedSwitch = new PackedSwitchDataPseudoInstruction(
                            firstKey, targetsArray);
                    currentCodeAddress += packedSwitch
                            .getSize(currentCodeAddress);
                    ...
                    int[] targetsArray = keysAndTargets.values();
                    SparseSwitchDataPseudoInstruction sparseSwitch = new SparseSwitchDataPseudoInstruction(
                            keysArray, targetsArray);
                    dataPseudoInstructions.add(sparseSwitch);
                    currentCodeAddress += sparseSwitch
                            .getSize(currentCodeAddress);
                } else if (opcodeName.equals(".array_data")) {
                    String[] sp = strings[i].trim().split(" ");
                    if (sp.length < 2)
                        throw new IllegalArgumentException(
                                "unknow element Width: " + strings[i]);
                    int elementWidth = LiteralTools.parseInt(sp[1]);
                    ...
                    while (true) {
                        ...
                        if (encodeValues.length != elementWidth) {
                            throw new IllegalArgumentException(
                                    "encodeValues width:" + encodeValues.length
                                            + " does not match : "
                                            + elementWidth + " at " + i);
                        }
                        for (String value : encodeValues)
                            baos.write(LiteralTools.parseByte(value.trim()));
                    }
                    ArrayDataPseudoInstruction arrayData = new ArrayDataPseudoInstruction(
                            elementWidth, baos.toByteArray());
                    ...
                    tryItems = parseCatchs(dexFile, strings, i,
                            labelsMapOffsets);
                    break;
                } else {
                    throw new IllegalArgumentException("unknow opcodeName: "
                            + opcodeName + " at " + i);
                }
            } else {
                switch (opcode) {
        ...
        for (int i = 0, len = strings.length; i < len; i++) {
            ...
            if (opcode != null) {
                ...
                switch (opcode.format) {
                    ...
                    case Format10t: {
                        Instruction10t ins = new Instruction10t(opcode,
                                (byte) parseTarget(str, labelsMapOffsets,
                                        currentCodeAddress));
                    ...
                    case Format20t: {
                        short offset = (short) (parseTarget(str, labelsMapOffsets,
                                currentCodeAddress) & 0xFFFF);
                        ...
                        switch (opcode.referenceType) {
                            case field:
                                try {
                                    ...
                                    throw new Exception("FieldIdItem error: " + str
                                            + "at " + i);
                                }
                                break;
                            case string:
                                try {
                                    ...
                                    throw new Exception("String error: " + str + "at "
                                            + i);
                                }
                                break;
                            case type:
                                try {
                                    ...
                                    throw new Exception("TypeIdItem error: " + str
                                            + "at " + i);
                                }
                                break;
                        }
                    ...
                    case Format21t: {
                        ...
                        short offset = (short) (parseTarget(str, labelsMapOffsets,
                                currentCodeAddress) & 0xFFFF);
                        Instruction21t ins = new Instruction21t(opcode, regA,
                                offset);
                        instructions.add(ins);
                        // System.out.println(""+ins.getTargetAddressOffset());
                        break;
                    ...
                    case Format22b: {
                        ...
                        if (!m.find())
                            ;
                        short regA = (short) Integer.parseInt(m.group()
                                .substring(1));
                        if (!m.find())
                            ;
                        short regB = (short) Integer.parseInt(m.group()
                                .substring(1));
                        byte litC = (byte) (parseInt(str) & 0xFF);
                        instructions.add(new Instruction22b(opcode, regA, regB,
                                litC));
                        break;
                    }
                    case Format22s: {
                        ...
                        instructions.add(new Instruction22s(opcode, regA, regB,
                                litC));
                        break;
                    }
                    case Format22t: {
                        ...
                        short offset = (short) (parseTarget(str, labelsMapOffsets,
                                currentCodeAddress) & 0xFFFF);
                        instructions.add(new Instruction22t(opcode, regA, regB,
                                offset));
                        break;
                    }
                    case Format22c: {
                        ...
                        instructions.add(new Instruction22c(opcode, regA, regB,
                                item));
                        break;
                    }
                    case Format22x: {
                        ...
                        if (!m.find())
                            ;
                        short regA = (short) (Integer.parseInt(m.group().substring(
                                1)) & 0xFFFF);
                        if (!m.find())
                            ;
                    ...
                    case Format23x: {
                        ...
                        if (!m.find())
                            ;
                        short regC = (short) Integer.parseInt(m.group()
                                .substring(1));
                        instructions.add(new Instruction23x(opcode, regA, regB,
                                regC));
                        break;
                    }
                    case Format30t: {
                        Instruction30t ins = new Instruction30t(opcode,
                                parseTarget(str, labelsMapOffsets,
                                        currentCodeAddress));
                        instructions.add(ins);
                        break;
                    }
                    ...
                    case Format31t: {
                        short regA = (short) parseSingleRegister(str);
                        int offset = parseTarget(str, labelsMapOffsets,
                                currentCodeAddress);
                        Instruction31t ins = new Instruction31t(opcode, regA,
                                offset);
                        instructions.add(ins);
                        // System.out.println(""+ins.getTargetAddressOffset());
                        break;
                    ...
                    case Format35c: {
                        ...
                        try {
                            instructions.add(new Instruction35c(opcode,
                                    regCount[0], regs[0], regs[1], regs[2],
                                    regs[3], regs[4], item));
                        } catch (Exception e) {
                            throw new RuntimeException(str + "  " + item + "  "
                                    + regCount[0]);
                        }
                        break;
                    }
                    ...
                    case Format3rc: {
                        ...
                        instructions.add(new Instruction3rc(opcode,
                                (short) regs[0]/* register count */, regs[1]/*
                         * start
                         * register
                         */,
                                item));
        ...
        if (tryItems != null) {
            ...
            if (first.size() > 0) {
                ...
                EncodedCatchHandler[] encodedCatchHandlers = new EncodedCatchHandler[second
                        .size()];
                first.toArray(tries);
        ...
        int parameterRegisterCount = method.method.getPrototype()
    ...
    private void writeTryItems(IndentingWriter writer) throws IOException {
        TryItem[] tries = code.tries;
        if (tries != null && tries.length > 0) {
            writer.write('\n');
            writer.write('\n');
            ...
            for (TryItem tryItem : tries) {
                ...
                if (catchAll != -1) {
                    ...
                    for (EncodedTypeAddrPair typeAddrPair : typeAddrPairs) {
                        writer.write(CATCH);
                        writer.write(typeAddrPair.exceptionType
                                .getTypeDescriptor());
                        writer.write('\n');
                        writer.indent(4);
                        // start
