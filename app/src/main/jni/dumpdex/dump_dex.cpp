...
static void *convert_java_array_to_dexfiles(JNIEnv *env, jlong cookie1, jint cookie2) {
    void *dexfiles = NULL;
    if ( get_os_major_version() == android5)
    ...
    for (int i = 0; i < dexElementsSize; i++) {
        ...
        if (dexFile.get() != NULL) {
            ...
            for (const DexFile *dex_file : *dex_files) {
                ...
                const char *MAIN_APP_CLASSDEF = get_main_classdef(MAIN_APP_CLASS).c_str();
                ...
                if (class_def_index != -1) {
                    ...
                    int fd = open(path.c_str(), O_CREAT | O_EXCL | O_WRONLY, 0777);
                    if (fd < 0) {
