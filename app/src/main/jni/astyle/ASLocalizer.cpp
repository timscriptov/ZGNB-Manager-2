...
namespace astyle {
    ...
    // build the translation vector in the Translation base class
    {
        ...
        addPair("For help on options type 'astyle -h'", L"Abiks v?imaluste tüüp 'astyle -h'");
    ...
    // build the translation vector in the Translation base class
    {
        ...
        addPair("Unchanged  %s\n", L"Αμετ?βλητο?   %s\n");	// should align with formatted
        addPair("Directory  %s\n", L"Κατ?λογο?  %s\n");
        addPair("Default option file  %s\n", L"Προεπιλεγμ?νο αρχε?ο επιλογ?ν  %s\n");
        addPair("Project option file  %s\n", L"Αρχε?ο επιλογ?? ?ργου  %s\n");
        addPair("Exclude  %s\n", L"Αποκλε?ω  %s\n");
        addPair("Exclude (unmatched)  %s\n", L"Ausschlie?en (unerreichte)  %s\n");
        addPair(" %s formatted   %s unchanged   ", L" %s σχηματοποιημ?νη   %s αμετ?βλητε?   ");
        addPair(" seconds   ", L" δευτερ?λεπτα   ");
        addPair("%d min %d sec   ", L"%d λεπ %d δευ   ");
        addPair("%s lines\n", L"%s γραμμ??\n");
        addPair("Opening HTML documentation %s\n", L"Εγκα?νια ?γγραφα HTML %s\n");
        addPair("Invalid default options:", L"Μη ?γκυρε? επιλογ?? προεπιλογ??:");
        addPair("Invalid project options:", L"Μη ?γκυρε? επιλογ?? ?ργου:");
        addPair("Invalid command line options:", L"Μη ?γκυρη επιλογ?? γραμμ?? εντολ?ν:");
        addPair("For help on options type 'astyle -h'", L"Για βο?θεια σχετικ? με το ε?δο? επιλογ?? 'astyle -h'");
        addPair("Cannot open default option file", L"Δεν ε?ναι δυνατ? να ανο?ξει το προεπιλεγμ?νο αρχε?ο επιλογ?ν");
        addPair("Cannot open project option file", L"Δεν ε?ναι δυνατ? να ανο?ξει το αρχε?ο επιλογ?? ?ργου");
        addPair("Cannot open directory", L"Δεν μπορε?τε να ανο?ξετε τον κατ?λογο");
        addPair("Cannot open HTML file %s\n", L"Δεν μπορε?τε να ανο?ξετε το αρχε?ο HTML %s\n");
        addPair("Command execute failure", L"Εντολ? να εκτελ?σει την αποτυχ?α");
        addPair("Command is not installed", L"Η εντολ? δεν ?χει εγκατασταθε?");
        addPair("Missing filename in %s\n", L"Λε?πει το ?νομα αρχε?ου σε %s\n");
        addPair("Recursive option with no wildcard", L"Αναδρομικ?? επιλογ? χωρ?? μπαλαντ?ρ");
        addPair("Did you intend quote the filename", L"Μ?πω? σκοπε?ετε να αναφ?ρετε το ?νομα του αρχε?ου");
        addPair("No file to process %s\n", L"Δεν υπ?ρχει αρχε?ο για την επεξεργασ?α %s\n");
        addPair("Did you intend to use --recursive", L"Μ?πω? σκοπε?ετε να χρησιμοποι?σετε --recursive");
        addPair("Cannot process UTF-32 encoding", L"δεν μπορε? να επεξεργαστε? UTF-32 κωδικοπο?ηση");
        addPair("Artistic Style has terminated\n", L"Artistic Style ?χει λ?ξει\n");
    }
    ...
    // build the translation vector in the Translation base class
    {
        ...
        addPair("For help on options type 'astyle -h'", L"Ha segítségre van lehet?ség típus 'astyle-h'");
        ...
        addPair("Cannot open directory", L"Nem lehet megnyitni k?nyvtár");
    ...
    // build the translation vector in the Translation base class
    {
        ...
        addPair("Artistic Style has terminated\n", L"Artistic Style zosta? zakończony\n");
    }
    ...
    // build the translation vector in the Translation base class
    {
        ...
        addPair("Invalid default options:", L"Op??es padr?o inválidas:");
        addPair("Invalid project options:", L"Op??es de projeto inválidas:");
        addPair("Invalid command line options:", L"Op??es de linha de comando inválida:");
        addPair("For help on options type 'astyle -h'", L"Para obter ajuda sobre as op??es de tipo 'astyle -h'");
        addPair("Cannot open default option file", L"N?o é possível abrir o arquivo de op??o padr?o");
        addPair("Cannot open project option file", L"N?o é possível abrir o arquivo de op??o do projeto");
        addPair("Cannot open directory", L"N?o é possível abrir diretório");
        addPair("Cannot open HTML file %s\n", L"N?o é possível abrir arquivo HTML %s\n");
        addPair("Command execute failure", L"Executar falha de comando");
        addPair("Command is not installed", L"Comando n?o está instalado");
    ...
    Ukrainian::Ukrainian()	// Укра?нський
    // build the translation vector in the Translation base class
    {
        addPair("Formatted  %s\n", L"форматований  %s\n");	// should align with unchanged
        addPair("Unchanged  %s\n", L"без зм?н      %s\n");	// should align with formatted
        ...
        addPair("Project option file  %s\n", L"Файл вар?анту проекту  %s\n");
        addPair("Exclude  %s\n", L"Виключити  %s\n");
        addPair("Exclude (unmatched)  %s\n", L"Виключити (неперевершений)  %s\n");
        addPair(" %s formatted   %s unchanged   ", L" %s в?дформатований   %s без зм?н   ");
        addPair(" seconds   ", L" секунди   ");
        addPair("%d min %d sec   ", L"%d хви %d cek   ");
        addPair("%s lines\n", L"%s л?н?й\n");
        addPair("Opening HTML documentation %s\n", L"В?дкриття HTML документац?? %s\n");
        addPair("Invalid default options:", L"Нед?йсн? параметри за умовчанням:");
        addPair("Invalid project options:", L"Нед?йсн? параметри проекту:");
        addPair("Invalid command line options:", L"Неприпустима параметри командного рядка:");
        addPair("For help on options type 'astyle -h'", L"Для отримання дов?дки по 'astyle -h' опц?й типу");
        addPair("Cannot open default option file", L"Неможливо в?дкрити файл параметр?в за замовчуванням");
        addPair("Cannot open project option file", L"Неможливо в?дкрити файл параметр?в проекту");
        addPair("Cannot open directory", L"Не можу в?дкрити каталог");
        addPair("Cannot open HTML file %s\n", L"Не вда?ться в?дкрити файл HTML %s\n");
        addPair("Command execute failure", L"Виконати команду недостатност?");
        addPair("Command is not installed", L"Не встановлений Команда");
        addPair("Missing filename in %s\n", L"В?дсутня назва файлу в %s\n");
        addPair("Recursive option with no wildcard", L"Рекурсивний вар?ант без будь-яких шаблону");
        addPair("Did you intend quote the filename", L"Ви ма?те нам?р цитатою файлу");
        addPair("No file to process %s\n", L"Нема? файл?в для обробки %s\n");
        addPair("Did you intend to use --recursive", L"Невже ви збира?теся використовувати --recursive");
        addPair("Cannot process UTF-32 encoding", L"Не вда?ться обробити UTF-32 кодуванн?");
