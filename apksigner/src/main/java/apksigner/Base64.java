...
public class Base64 {
    ...
    @Contract("_, _, _ -> new")
    public static @NotNull String encode(byte[] data, int offset, int length) {
        int i;
        int encodedLen;
        char[] encoded;
    ...
    public static @NotNull String encode(byte[] data) {
        return encode(data, 0, data.length);
    }

    private static void encodeQuantum(@NotNull byte in[], int inOffset, int len,
                                      @NotNull char out[], int outOffset) {
        byte a = 0, b = 0, c = 0;
        a = in[inOffset];
    ...
    public static byte @NotNull [] decode(String encoded, int offset, int length)
            throws IOException {
        int i;
        int decodedLen;
        byte[] decoded;
