...
//        mContentView.setTextViewText(R.id.artist, getString(R.string.no_artist));
    ...
    private NotificationCompat.@NotNull Builder createBuilder(Context context, int id) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
    ...
    private NotificationCompat.@NotNull Builder createBuilderO(@NotNull Context context, int id) {
...
//        Log.d("AudioView", "broadcast: " + type + " tag: " + mAttachedTag);
    ...
    public void setDataSource(@NotNull Object dataSource) {
        try {
    ...
    public void setDataSource(@NotNull List tracks) throws RuntimeException {
        if (tracks.size() > 0) {
    ...
    public void start() {
        mContentView.setImageViewResource(R.id.play, R.drawable.ic_pause);
        mNotificationManager.notify(AUDIO_SERVICE_NOTIFICATION, mBuilder.build());
        if (mIsPrepared) {
            try {
                mMediaPlayer.start();
    ...
    public void pause() {
        mContentView.setImageViewResource(R.id.play, R.drawable.ic_play);
        mNotificationManager.notify(AUDIO_SERVICE_NOTIFICATION, mBuilder.build());
        try {
